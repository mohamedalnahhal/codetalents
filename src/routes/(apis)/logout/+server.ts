import { jwtExpirationSeconds } from "$lib/server/auth/jwt";
...
export async function GET({ url, cookies, locals }) {
  if (locals.session) {
    ...
    cookies.delete("session_jwt", { path: '/', httpOnly: true, maxAge: jwtExpirationSeconds, secure: true, sameSite: 'lax' });
    locals.session = null;
  }
