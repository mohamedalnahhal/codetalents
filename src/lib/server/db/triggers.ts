...
async function main() {
  ...
  // creating triggers
  db.execute(
sql`CREATE OR REPLACE FUNCTION trg_attempt_update()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER IF NOT EXISTS trig_attempt
AFTER INSERT ON user_submission
FOR EACH ROW EXECUTE FUNCTION trg_attempt_update();

CREATE TRIGGER IF NOT EXISTS trig_solved
AFTER INSERT ON user_submission
FOR EACH ROW EXECUTE FUNCTION trg_solved_update();

CREATE OR REPLACE FUNCTION trg_contest_attempt_update()
RETURNS TRIGGER AS $$
BEGIN
    -- If user has NOT attempted before → increment attemptCount
    IF NOT EXISTS (
        SELECT 1 FROM user_contest_submission
        WHERE contest_problem_id = NEW.contest_problem_id
        AND username = NEW.username
        AND (contest_problem_id, username) <> (NEW.contest_problem_id, NEW.username)
    ) THEN
        UPDATE contest_problem
        SET contest_attempt_count = contest_attempt_count + 1
...
CREATE OR REPLACE FUNCTION trg_contest_solved_update()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.status = 'Accepted' THEN
        -- Check if user already solved it
        IF NOT EXISTS (
            ...
            UPDATE contest_problem
            SET contest_solved_count = contest_solved_count + 1
            WHERE id = NEW.contest_problem_id;
        END IF;
    END IF;
...
CREATE TRIGGER IF NOT EXISTS trig_contest_attempt
AFTER INSERT ON user_contest_submission
FOR EACH ROW EXECUTE FUNCTION trg_contest_attempt_update();

CREATE TRIGGER IF NOT EXISTS trig_contest_solved
AFTER INSERT ON user_contest_submission
FOR EACH ROW EXECUTE FUNCTION trg_contest_solved_update();
